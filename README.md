...
- `EPiServer.ContentDeliveryApi.Cms` - This package provides the necessary components and functionality to enable content delivery through APIs in an Optimizely CMS application. It allows developers to create and manage content that can be accessed and consumed by external applications or services via APIs.
...
- `Optimizely.ContentGraph.Cms` - This package provides a content graph API for Optimizely CMS, allowing developers to query and retrieve content in a structured and efficient manner. It enables developers to build complex queries and retrieve content based on relationships and properties, making it easier to access and manipulate content within the CMS.
